...
class PoseDetector(object):
    ...
    def __inference_fast(self, img, net_input_height_size, stride, upsample_ratio):
        ...
        scaled_img = cv2.resize(img, (0, 0), fx=self.scale, fy=self.scale, interpolation=cv2.INTER_LINEAR)
        ...
        self.avg_heatmap = cv2.resize(heatmaps, (0, 0), fx=upsample_ratio, fy=upsample_ratio,
                                      interpolation=cv2.INTER_CUBIC)

        stage2_pafs = stages_output[-1]
        pafs = np.transpose(stage2_pafs.squeeze().cpu().data.numpy(), (1, 2, 0))
        self.avg_paf = cv2.resize(pafs, (0, 0), fx=upsample_ratio, fy=upsample_ratio, interpolation=cv2.INTER_CUBIC)

        total_keypoints_num = 0
        all_keypoints_by_type = []
        for kpt_idx in range(self.num_keypoints):  # 19th for bg
            total_keypoints_num += extract_keypoints(self.avg_heatmap[:, :, kpt_idx], all_keypoints_by_type,
                                                     total_keypoints_num)

        self.pose_entries, self.all_keypoints = group_keypoints(all_keypoints_by_type, self.avg_paf)
    ...
    def visualize_prediction(self, image):
        ...
        if not np.array_equal(self.image, image):
            self.image = image
            self.__inference_fast(self.image, self.height_size, self.stride, self.upsample_ratio)
        ...
        for n in range(len(self.pose_entries)):
            ...
            for kpt_id in range(self.num_keypoints):
                if self.pose_entries[n][kpt_id] != -1.0:  # keypoint was found
                    pose_keypoints[kpt_id, 0] = int(self.all_keypoints[int(self.pose_entries[n][kpt_id]), 0])
                    pose_keypoints[kpt_id, 1] = int(self.all_keypoints[int(self.pose_entries[n][kpt_id]), 1])
    ...
    def __visualize_prediction_slow(self, image):
        ...
        pose_entries, all_keypoints = group_keypoints(all_keypoints_by_type, self.avg_paf)
